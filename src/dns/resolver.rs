...
impl Resolver {
    ...
    pub fn try_new() -> Result<Self, anyhow::Error> {
        let resolver = hickory_resolver::Resolver::builder_tokio()
            ...
            .build()?;
        Ok(Self::__new(resolver))
    }

    /// Create a new DNS resolver using the provided nameservers for DNS resolution
    pub fn new_with_upstream(nameservers: Vec<NameServerConfig>) -> Result<Self, anyhow::Error> {
...
impl Resolver {
    ...
    #[inline]
    fn __new(resolver: hickory_resolver::Resolver<TokioRuntimeProvider>) -> Self {
        Self { resolver }
    }
...
pub enum Error {
    ...
    LookupFailure(#[from] hickory_resolver::net::NetError),
}
...
mod tests {
    ...
    async fn test_resolve_cname_chain(#[case] source: &str, #[case] expected: &str) {
        ...
        let resolver = Resolver::try_new().unwrap();
